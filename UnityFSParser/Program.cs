...
class Program
{
    ...
    {
        List<MemoryAssetsFile> MemoryAssetsFileList = new List<MemoryAssetsFile>();
        ...
        using (BinaryReader reader = new BinaryReader(File.Open(@"file.unity3d", FileMode.Open)))
        {
            ...
            if (sigString != "UnityFS")
                throw new Exception("Not a UnityFS header");
            var fileVer = reader.ReadUInt32BE();
            var playerVer = reader.ReadAsciiNullTerminatedString();
            var feVersion = reader.ReadAsciiNullTerminatedString();

            var totalFileSize = reader.ReadUInt64BE();
            var compressedBlockSize = reader.ReadUInt32BE();
            var decompBlockSize = reader.ReadUInt32BE();
            ...
            {
                byte[] uncompressedBytes = new byte[decompBlockSize];
                using (var mstream = new MemoryStream(blocksInfoBytes))
                {
                    var decoder = new Lz4DecoderStream(mstream);
                    decoder.Read(uncompressedBytes, 0, (int)decompBlockSize);
                    decoder.Dispose();
                }

                blockReader = new BinaryReader(new MemoryStream(uncompressedBytes));
            }
            
            blockReader.ReadBytes(0x10);
            int blockCount = blockReader.ReadInt32BE();
            var assetsDataStream = new MemoryStream();
            for (int i = 0; i < blockCount; i++)
            {
                var uncompressedSize = blockReader.ReadInt32BE();
                var compressedSize = blockReader.ReadInt32BE();
                var flag = blockReader.ReadInt16BE();
            ...
            var assetsData = new BinaryReader(assetsDataStream);
            using (assetsData)
            {
                var entryinfo_count = blockReader.ReadInt32BE();
                for (int i = 0; i < entryinfo_count; i++)
                {
                    var memFile = new MemoryAssetsFile();
                    var entryinfo_offset = blockReader.ReadInt64BE();
                    var entryinfo_size = blockReader.ReadInt64BE();
                    var unknown = blockReader.ReadInt32BE();
                    memFile.fileName = blockReader.ReadAsciiNullTerminatedString();
                    assetsData.BaseStream.Seek(entryinfo_offset, SeekOrigin.Begin);
//                    var buffer = new byte[entryinfo_size];
            ...
            foreach(var  memFile in MemoryAssetsFileList)
            {
                using (FileStream fs = File.OpenWrite(memFile.fileName + ".asset"))
                {
                    memFile.memStream.CopyTo(fs);
                }
            }            
        }
    }
